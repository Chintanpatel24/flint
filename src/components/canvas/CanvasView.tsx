...
import { Grip, RotateCcw, Search, X, Plus, Type, Trash2, FileText, Globe, Layers, Navigation, Maximize2, Pencil } from 'lucide-react';
...
interface WebCardBodyProps {
  card: CanvasCard;
  zoom: number;
  updateCard: (id: string, updates: Partial<CanvasCard>) => void;
}

function WebCardBody({ card, zoom, updateCard }: WebCardBodyProps) {
  const [url, setUrl] = useState(card.url || '');
  const [isEditing, setIsEditing] = useState(!card.url);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateCard(card.id, { url: finalUrl });
    setIsEditing(false);
  };

  if (isEditing) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-4 gap-3 bg-[#09090b]">
        ...
        <form onSubmit={handleSubmit} className="w-full flex flex-col gap-2">
          <input
            ...
            className="w-full bg-[#18181a] border border-[#2e2e31] rounded px-3 py-2 text-sm text-white focus:outline-none focus:border-[#7f6df2]"
          />
          <button type="submit" className="bg-[#7f6df2] text-white text-xs font-semibold py-2 rounded hover:bg-[#6b5cd9]">
            Embed Website
          </button>
        </form>
      </div>
...
interface RichTextCardBodyProps {
  card: CanvasCard;
  zoom: number;
  updateCard: (id: string, updates: Partial<CanvasCard>) => void;
...
function RichTextCardBody({ card, zoom, updateCard, pushHistorySnapshot }: RichTextCardBodyProps) {
  const editor = useEditor({
    extensions: [StarterKit, Markdown],
    content: card.content || '',
    onUpdate: ({ editor }) => {
      // @ts-ignore
      updateCard(card.id, { content: editor.storage.markdown.getMarkdown() });
    },
    editorProps: {
      attributes: {
        class: 'canvas-tiptap-editor prose prose-invert max-w-none focus:outline-none text-[12px]',
  ...
  useEffect(() => {
    if (editor && editor.storage.markdown.getMarkdown() !== card.content) {
      editor.commands.setContent(card.content || '');
    }
  }, [card.content, editor]);

  return (
    <div className="flex-1 overflow-auto canvas-card-scroll p-2">
...
export function CanvasView() {
  ...
  const handleDrop = (e: React.DragEvent) => {
    e.preventDefault();
    const noteId = e.dataTransfer.getData('text/flint-note-id');
    if (noteId) {
      const rect = containerRef.current!.getBoundingClientRect();
    ...
    >
      ...
      >
        ...
        {filteredCards.map(card => {
          ...
          const isWeb = card.type === 'web';
          const isFrame = card.type === 'group';
          ...
          return (
            ...
            >
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0, flex: 1 }}>
                  ...
                  {isNote ? (
                    ...
                    <Globe size={11} style={{ color: colorIdx > 0 ? cardColor.border : '#00b894', flexShrink: 0 }} />
                  ) : isFrame ? (
              ...
              {!isFrame && (
                <div style={{ padding: (isImage || isWeb) ? 0 : '0', flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                  {isNote ? (
                    <div style={{
                      fontSize: 11.5, color: textSecondary, lineHeight: 1.5,
                      whiteSpace: 'pre-wrap', wordBreak: 'break-word', overflowY: 'auto',
                      fontFamily: 'Inter, -apple-system, sans-serif',
                      padding: '8px 10px', paddingRight: 4,
        ...
        >
          ...
          <div style={{ padding: '4px 12px' }}>
            <input
              ...
              value={connections.find(c => c.id === lineContextMenu.connId)?.label || ''}
              onChange={(e) => {
                const label = e.target.value;
                setConnections(prev => prev.map(c => c.id === lineContextMenu.connId ? { ...c, label } : c));
              }}
              style={{
                width: '100%',
                background: '#0a0a0a',
        ...
        >
          <div style={{ width: '100%', height: '100%', position: 'relative' }}>
            {(() => {
              ...
              const scale = Math.min(160 / rangeX, 120 / rangeY);
              
              const offsetX = (160 - rangeX * scale) / 2;
                ...
                <>
                  ...
                  {cards.map(c => (
                    <div
                      ...
                      style={{
                        position: 'absolute',
                        left: offsetX + (c.x - minX) * scale,
                        top: offsetY + (c.y - minY) * scale,
                        width: Math.max(2, c.w * scale),
                        height: Math.max(2, c.h * scale),
                        background: selectedCard === c.id ? accentColor : 'rgba(255,255,255,0.2)',
                        borderRadius: 1,
                      }}
                    />
                  ))}
                  ...
                  <div
                    style={{
                      position: 'absolute',
                      left: offsetX + (-pan.x / zoom - minX) * scale,
                      top: offsetY + (-pan.y / zoom - minY) * scale,
      ...
      >
        ...
        <button
          ...
          style={{
            background: 'none', border: 'none', color: showMiniMap ? accentColor : textSecondary,
            cursor: 'pointer', width: 24, height: 24, borderRadius: 4,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            transition: 'background-color 0.15s, color 0.15s',
          }}
