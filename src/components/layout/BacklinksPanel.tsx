...
import { Link2, ChevronRight, FileText, Waypoints, ArrowLeft, ArrowRight } from 'lucide-react';
...
export function BacklinksPanel({ noteId }: { noteId: string }) {
  ...
  return (
    <div className="animate-slide-right flex flex-col shrink-0"
      ...
      <div style={{ height: 200, borderBottom: '1px solid #1a1a1a' }}>
        <LocalGraphView noteId={noteId} />
      </div>

      {/* Outline */}
      {headings.length > 0 && (
        <div style={{ padding: '12px 12px 8px' }}>
          <div style={{ fontSize: 10, fontWeight: 600, color: '#333', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>{t('common.outline')}</div>
          {headings.map((h, i) => (
      ...
      <div style={{ padding: '8px 12px', borderTop: headings.length ? '1px solid #1a1a1a' : 'none' }}>
        <div style={{ fontSize: 10, fontWeight: 600, color: '#333', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>
          {t('common.backlinks')} ({backlinks.length})
        </div>
      ...
      <div style={{ padding: '8px 12px', borderTop: '1px solid #1a1a1a' }}>
        <div style={{ fontSize: 10, fontWeight: 600, color: '#333', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 6 }}>
          {t('common.outgoing')} ({outgoing.length})
        </div>
